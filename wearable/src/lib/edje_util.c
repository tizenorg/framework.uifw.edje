...
// TIZEN ONLY
static Eina_Hash *styles_cache = NULL;
...
edje_text_class_set(const char *text_class, const char *font, Evas_Font_Size size)
{
   ...
   while (members)
	...
	ed->dirty = EINA_TRUE;
        ed->recalc_call = EINA_TRUE;
   if (ed->file) _edje_styles_cache_del(ed->file->path);
...
// ********************* TIZEN ONLY START
static void
_edje_styles_cache_free(void *data)
{
   eina_hash_free(data);
}

void
_edje_styles_cache_init()
{
   if (!styles_cache)
     styles_cache = eina_hash_string_small_new(_edje_styles_cache_free);
}

void
_edje_styles_cache_shutdown()
{
   if (styles_cache)
     {
        eina_hash_free(styles_cache);
        styles_cache = NULL;
     }
}
...
_edje_styles_cache_clean()
{
   if (styles_cache)
     {
        eina_hash_free_buckets(styles_cache);
     }
}

Eina_Hash *
_edje_styles_cache_find(const char *key)
{
   return eina_hash_find(styles_cache, key);
}

void
_edje_styles_cache_del(const char *key)
{
   Eina_List *tmp;
   tmp = eina_hash_find(styles_cache, key);
   if (tmp)
      eina_hash_del_by_key(styles_cache, key);
}

void
_edje_styles_cache_add(const char *key, Eina_Hash *styles)
{
   _edje_styles_cache_del(key);
   eina_hash_add(styles_cache, key, styles);
}
// ************************* TIZEN ONLY END
