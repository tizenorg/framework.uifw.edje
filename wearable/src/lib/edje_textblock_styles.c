...
static void
_edje_style_cache_free(void *data)
{
   free(data);
}
...
// TIZEN ONLY
   Eina_Hash *styles = NULL;
   styles = _edje_styles_cache_find(ed->file->path);
   if (styles)
     {
       EINA_LIST_FOREACH(ed->file->styles, l, stl)
         {
           char *tmp = eina_hash_find(styles, stl->name);
           if (tmp)
           evas_textblock_style_set(stl->style, tmp);
         }
       return;
     }
   styles = eina_hash_string_small_new(_edje_style_cache_free);
// --------------
...
// TIZEN ONLY
	if (styles && eina_strbuf_string_get(txt))
      {
         char *tmp = strdup(eina_strbuf_string_get(txt));
         eina_hash_add(styles, stl->name, tmp);
      }
// -----------
...
// TIZEN ONLY
   if (styles)
     _edje_styles_cache_add(ed->file->path, styles);
// ------------
}
