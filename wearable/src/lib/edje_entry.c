...
_text_filter_markup_prepend_internal(Entry *en, Evas_Textblock_Cursor *c, char *text)
{
   ...
   en->prepended_len = evas_string_char_len_get(evas_textblock_text_markup_to_utf8(NULL, text));
   if (text)
     {
...
// TIZEN ONLY - START
static int
_get_margin_value(const char *format, const char *tag)
{
   int ret, count;
   char *start, *end, *val;
   ...
   val[count] = '\0';
   if ((strlen(val) == 5) && (!strcmp(val, "reset")))
     {
        free(val);
        return ret;
     }
   ...
   if (en->have_selection)
     {
        ...
        int rm, urm;
        const Evas_Textblock_Style *ts, *uts;
        ...
        if (ts)
           rm = _get_margin_value(evas_textblock_style_get(ts), "right_margin");
        uts = evas_object_textblock_style_user_peek(en->rp->object);
        if (uts)
           urm = _get_margin_value(evas_textblock_style_get(uts), "right_margin");
        rm = rm > urm ? rm : urm;
        ...
        EINA_LIST_FOREACH(en->sel, l, sel)
          {
             ...
             Evas_Coord ew;
             Eina_List *range_next;
             Evas_Textblock_Rectangle *r_next = NULL;
             list_idx++; // TIZEN ONLY

             evas_object_geometry_get(en->rp->object, NULL, NULL, &ew, NULL);
             r = range->data;
             // TIZEN ONLY - START
             range_next = eina_list_next(range);
             if (range_next)
                r_next = range_next->data;

             if (sel->obj_bg)
               {
                  evas_object_move(sel->obj_bg, x + r->x, y + r->y);
                  if ((list_idx != list_cnt) && (r_next) && (r->y != r_next->y))
                     evas_object_resize(sel->obj_bg, ew - r->x - rm, r->h);
                  else
             ...
             if (sel->obj_fg)
               {
                  ...
                  if ((list_idx != list_cnt) && (r_next) && (r->y != r_next->y))
                     evas_object_resize(sel->obj_fg, ew - r->x - rm, r->h);
                  else
                                 ...
                                 if (en->layout_region.w != -1 && en->layout_region.h != -1 &&
                                     ...
                                     || (ny > en->viewport_region.y + en->viewport_region.h + moving)))
                                   {
                                      if ((en->layout_region.w != -1) &&
                                          (nx >= en->layout_region.x + en->layout_region.w - bh_gap))
        ...
        if (string)
          {
             ...
             if ((rp->part->entry_mode == EDJE_ENTRY_EDIT_MODE_PASSWORD) &&
                 _edje_password_show_last)
               {
                  _edje_entry_hide_visible_password(en->rp);
                  buf = eina_strbuf_new();
                  eina_strbuf_append_printf(buf, "<password=off>%s</password>",
                                            evas_textblock_text_utf8_to_markup(NULL, string));
   ...
   if (en->rp->part->select_mode == EDJE_ENTRY_SELECTION_MODE_BLOCK_HANDLE)
     {
        ...
        x = en->dx - x;
        y = en->dy - y;
        evas_textblock_cursor_char_coord_set(en->cursor, x, y);
...
_edje_entry_start_handler_mouse_up_cb(void *data, Evas *e __UNUSED__, Evas_Object *obj __UNUSED__, void *event_info)
{
   ...
   if (evas_textblock_cursor_compare(en->sel_start, en->sel_end) > 0)
     ...
     {
        // last char case
        evas_textblock_cursor_char_prev(en->sel_start);
     }
   //update handlers
...
{
   Entry *en;
   Evas_Coord hx, hy, edx, edy, edw, edh;
   if ((rp->type != EDJE_RP_TYPE_TEXT) ||
       (!rp->typedata.text)) return;
   en = rp->typedata.text->entry_data;
   if (!en) return;
...
_edje_entry_imf_event_commit_cb(void *data, Ecore_IMF_Context *ctx __UNUSED__, void *event_info)
{
   ...
   if ((rp->part->entry_mode == EDJE_ENTRY_EDIT_MODE_PASSWORD) &&
       _edje_password_show_last && (!en->preedit_start))
     {
        buf = eina_strbuf_new();
        eina_strbuf_append_printf(buf, "<password=off>%s</password>",
                                  evas_textblock_text_utf8_to_markup(NULL, commit_str));
        _text_filter_markup_prepend(en, en->cursor, eina_strbuf_string_get(buf));
        eina_strbuf_free(buf);
